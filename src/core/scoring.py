...
class Scoring:
    ...
    def rank(
        self, remaining: dict[int, list[int]], last_placed_piece=None
    ) -> list[PlayerScore]:
        scores = []
        for player_id, piece_ids in remaining.items():
            total = sum(piece_square_count(self.catalog, pid) for pid in piece_ids)
            scores.append(
                PlayerScore(player_id=player_id, score=total, is_winner=False)
            )

        min_score = min(s.score for s in scores)
        scores = [
            PlayerScore(
                player_id=s.player_id, score=s.score, is_winner=(s.score == min_score)
            )
            for s in scores
        ]
