...
class JsonStateRepo:
        ...
        return json.dumps(
            {
                "config": {
                    ...
                    "starting_positions": {
                        ...
                        for pid, pos in sorted(
                            memento.config.starting_positions.items()
                        )
                    },
                },
    ...
    def restore(self, data: str) -> Memento:
        ...
        config = (
            ConfigBuilder()
            .with_board_dimensions(
                config_data["board_width"], config_data["board_height"]
            )
        ...
        board_state = tuple(
            tuple(cell for cell in row) for row in parsed["board_state"]
        )
        remaining_pieces = tuple(
            (item[0], tuple(item[1])) for item in parsed["remaining_pieces"]
        )
        is_first_move = tuple((item[0], item[1]) for item in parsed["is_first_move"])
        last_placed_piece = tuple(
            (item[0], item[1]) for item in parsed.get("last_placed_piece", [])
        )
        return Memento(
            config=config,
